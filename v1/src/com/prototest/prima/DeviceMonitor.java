...
public class DeviceMonitor {
	...
	public int max_num_processes = 20;
	...
	public ProcessStats[] getProcessStats()
	{
		...
		//List<ProcessStats> procStats = new ArrayList<ProcessStats>();
		
		try {

			Process p = Runtime.getRuntime().exec("top -m 20 -d 1 -n 1");
			p.waitFor();
			...
			while(line!=null)
			{
				...
				try {
					...
					stats.load = ((float)(Integer.parseInt(toks[2].replace("%",""))))/100;
	...
	{
		ProcessStats[] procs = new ProcessStats[20];
		procs = getProcessStats();
		String message = "";
		for (ProcessStats processStats : procs) {
			message += processStats.pid + " " + processStats.load + " " + processStats.memUsedKb + " " + processStats.numThreads + " " + processStats.name + " " + processStats.user + "\n"; 
		}
